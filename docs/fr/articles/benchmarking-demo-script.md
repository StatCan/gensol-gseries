...
denton_mult_td <- td(
  formula = ma_sc_ann ~ 0 + ma_sc_tri,
  method = "denton-cholette",
  criterion = "proportional"
)

comp_mult <- ts.union(
  gseries = tsDF_to_ts(denton_mult$series, frequency = 4),
...
comp_add <- ts.union(
  gseries = tsDF_to_ts(denton_add$series, frequency = 4),
...
mes_eta3 <- stack_bmkDF(
  ts_to_bmkDF(ts.union(A = ventes_ann, B = ventes_ann),ind_frequency = 4)
)
...
res_eta_AP_sansBiais <- benchmarking(
  mes_ind_AP,
  mes_eta_AP,
  rho = 0.9,
  lambda = 1,
...
plot_graphTable(
  res_eta_AP_sansBiais$graphTable, 
...
res_eta_AP_denton <- benchmarking(
  mes_ind_AP,
  mes_eta_AP,
  rho = 1,
  lambda = 1
...
res_eta_AP_approxDenton <- benchmarking(
  mes_ind_AP,
  mes_eta_AP,
  rho = 0.999,
  lambda = 1,
...
plot_graphTable(
  res_eta_AP_approxDenton$graphTable, 
...
res_eta_AP_prorata <- benchmarking(
  mes_ind_AP,
  mes_eta_AP,
  rho = 0,
  lambda = 0.5,
...
res_stock_SB <- stock_benchmarking(
  mes_ind_stock,
  mes_eta_stock,
  rho = 0.729,
  lambda = 1,
...
plot_benchAdj(
  PB_graphTable = res_stock_PB$graphTable,
...
plot(
  res_stock_SB$graphTable$t, res_stock_SB$graphTable$benchmarked, 
  type = "b", col = "red", xlab = "t", ylab = "Stocks étalonnés"
)
lines(
  res_stock_PB$graphTable$t, res_stock_PB$graphTable$benchmarked, 
  type = "b", col = "blue"
)
legend(
...
res_stock_SB2 <- stock_benchmarking(
  mes_ind_plat,
  mes_eta_stock,
  rho = 0.729,
  lambda = 1,
...
plot(
  res_stock_SB2$graphTable$t, res_stock_SB2$graphTable$benchmarked,
  type = "b", col = "red", xlab = "t", ylab = "Stocks étalonnés"
)
lines(
  res_stock_PB2$graphTable$t, res_stock_PB2$graphTable$benchmarked,
  type = "b", col = "blue"
)
legend(
  x = "topleft", bty = "n", inset = 0.05, lty = 1, pch = 1,
...
plot_benchAdj(
  PB_graphTable = res_stock_PB2$graphTable,
