...
td_mult_denton <- td(
  formula = my_ts_ann ~ 0 + my_ts_qtr,
  method = "denton-cholette",
  criterion = "proportional"
)

compare_mult <- ts.union(
  gseries = tsDF_to_ts(mult_denton$series, frequency = 4),
...
compare_add <- ts.union(
  gseries = tsDF_to_ts(add_denton$series, frequency = 4),
...
my_benchmarks3 <- stack_bmkDF(
  ts_to_bmkDF(ts.union(A = ann_sales, B = ann_sales), ind_frequency = 4)
)
...
ann_AP <- round(
  jitter(
    aggregate.ts(AirPassengers, nfrequency = 1, FUN = sum) * 5, amount = 2500
  )
)
...
out_bench_AP_noBias <- benchmarking(
  my_AP_ind,
  my_AP_bmk,
  rho = 0.9,
  lambda = 1,
...
plot_graphTable(
  out_bench_AP_noBias$graphTable, 
...
out_bench_AP_denton <- benchmarking(
  my_AP_ind,
  my_AP_bmk,
  rho = 1,
  lambda = 1
...
plot_graphTable(
  out_bench_AP_denton$graphTable, 
...
out_bench_AP_dentonApprox <- benchmarking(
  my_AP_ind,
  my_AP_bmk,
  rho = 0.999,
  lambda = 1,
...
plot_graphTable(
  out_bench_AP_dentonApprox$graphTable, 
...
out_bench_AP_proRate <- benchmarking(
  my_AP_ind,
  my_AP_bmk,
  rho = 0,
  lambda = 0.5,
...
plot_graphTable(
  out_bench_AP_proRate$graphTable, 
...
plot_benchAdj(
  PB_graphTable = out_stock_PB$graphTable,
...
plot(
  out_stock_SB$graphTable$t, out_stock_SB$graphTable$benchmarked, 
  type = "b", col = "red", xlab = "t", ylab = "Benchmarked Stock"
)
lines(
  out_stock_PB$graphTable$t, out_stock_PB$graphTable$benchmarked, 
  type = "b", col = "blue"
)
legend(
...
out_stock_SB2 <- stock_benchmarking(
  my_flat_ind,
  my_stock_bmk,
  rho = 0.729,
  lambda = 1,
...
plot(
  out_stock_SB2$graphTable$t, out_stock_SB2$graphTable$benchmarked, 
  type = "b", col = "red", xlab = "t", ylab = "Benchmarked Stock"
)
lines(
  out_stock_PB2$graphTable$t, out_stock_PB2$graphTable$benchmarked, 
  type = "b", col = "blue"
)
legend(
...
plot_benchAdj(
  PB_graphTable = out_stock_PB2$graphTable,
