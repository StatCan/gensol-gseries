    ...
    if (any(abs(discr_ini) > gs.min_tolerance)) {
      ...
      if (mean_Ve != 0) {
        ...
        if (length(all_binding_id) > 0) {
          if (P == 1) {
            quiet_msg_func(
              "All component series are binding (non alterable) for the following totals:",
            ...
            quiet_msg_func(
              "All component series are binding (non alterable) for the following totals (applicable periods in brackets):",
              paste0("\n  ",
                sapply(
                  unique(tot_id),
                  function(x) {
        ...
        if (V_factor == 0) {
          if (P == 1) {
            quiet_msg_func(
              "All component series are binding (non alterable). Nothing to solve. ",
              "Returning the input component series data (and resulting totals).\n"
            )
          } else {
            quiet_msg_func(
              "All component series are binding (non alterable) for all periods. Nothing to solve. ",
              "Returning the input component series data (and resulting totals).\n"
            )
          }
...
build_raking_problem <- function(data_df,
  ...
  if (M == 1) {
    ...
    # Totals alter coef vector c_g (column-major version of the alter coef data frame data)
    c_g <- c(
      unlist(create_alterDF(tot1_cols, alterTotal1), use.names = FALSE),
