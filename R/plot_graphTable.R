...
plot_graphTable <- function(graphTable,
  ...
  if (n_ser == 0) {
    ...
    for (ii in 1:n_ser) {
      ...
      out_list$graph_list <- c(
        out_list$graph_list,
        list(list(
          name = bookmarks_df$title[ii],
    ...
    if (display_flag) {
      ...
      message(
        "Benchmarking graphics generated for ", format(n_ser), " series in the following PDF file:\n  ",
...
ori_plot <- function(graphTable,
  ...
  original_scale <- ggplot2::ggplot() +
    ...
    # Series lines
    ggplot2::geom_line(data = lines_df, ggplot2::aes(
      x = .data$rDate, y = .data$value,
      color = .data$series
    )) +

    # Average lines (segments)
    ggplot2::geom_segment(data = segments_df, ggplot2::aes(
      x = .data$rDate, y = .data$value,
    ...
    ggplot2::geom_point(data = points_df, ggplot2::aes(
      x = .data$rDate, y = .data$value,
      color = .data$series,
      shape = .data$series,
      size = .data$series
...
adj_plot <- function(graphTable,
  ...
  labels <- c(
    "benchmarkedSubAnnualRatio" = "BI Ratios (Benchmarked Series / Indicator Series)",
    "avgBenchmarkSubAnnualRatio" = "Avgerage BI Ratios",
    "bias" = "Bias"
  )
  colors <- c(
    "benchmarkedSubAnnualRatio" = "blue",
    "avgBenchmarkSubAnnualRatio" = "black",
    "bias" = "green4"
  )
  ln_types <- c(
    "benchmarkedSubAnnualRatio" = "solid",
    "avgBenchmarkSubAnnualRatio" = "solid",
    "bias" = "longdash"
  )
  pt_shapes <- c(
    "benchmarkedSubAnnualRatio" = "plus",
    "avgBenchmarkSubAnnualRatio" = "triangle open",
    "bias" = ""
  )
  pt_sizes <- c(
    "benchmarkedSubAnnualRatio" = pt_sz,
...
GR_plot <- function(graphTable,
  ...
  labels <- c(
    "growthRateSubAnnual" = "Growth R. in Indicator Series",
    "growthRateBenchmarked" = "Growth R. in Benchmarked Series"
  )
  colors <- c(
    "growthRateSubAnnual" = "steelblue",
  ...
  for (kk in seq_beg_yr) {
    ...
    if (kk > beg_yr && end_yr < end) {
      ...
      bars_df <- rbind(bars_df, data.frame(
        year = rep((end_yr + 1):end, each = 2),
...
GR_table <- function(graphTable,
  ...
  GR_df <- graphTable[
    !duplicated(graphTable[c("year", "period")]),
