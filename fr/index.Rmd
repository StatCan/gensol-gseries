...
G<span>&#8209;</span>Séries 3.0 (librairie gseries 3.0.1) est la première version du logiciel offerte en libre accès (logiciel libre). Elle inclut le recodage en R de toutes les fonctionalités SAS^®^ de G<span>&#8209;</span>Séries 2.0, soient PROC BENCHMARKING, PROC TSRAKING et la macro ***GSeriesTSBalancing***, ainsi qu'une fonction pour l'étalonnage de *séries de stocks* par l'entremise d'interpolations par spline cubique où les noeuds de la spline correspondent aux ratios ou différences entre les valeurs des étalons et de la série indicatrice. Il comprend les *fonctions principales* suivantes:
