...
test_ts <- ts(
  matrix(rep(c(100, 3, 5, 10, 200), 11),
    ncol = 5,
    byrow = TRUE,
...
test_that("Lower/upper bounds mapping bug", {
  expect_equal(
    ...
    c("con", "a", "b")
  ) # early problematic versions would return `c("con", "c", "a")`
  expect_equal(
...
test_that("Default OSQP settings => warning", {
  expect_warning(suppressMessages(
    tsbalancing(window(test_ts, start = start(test_ts), end = start(test_ts)), test_specs,
      quiet = TRUE,
...
test_that("Default OSQP settings + forced polishing => normal", {
  expect_equal(suppressMessages(
    tsbalancing(window(test_ts, start = start(test_ts), end = start(test_ts)), test_specs,
      quiet = TRUE,
      osqp_settings_df = data.frame(polish = TRUE)
    )$osqp_settings_df$polish
  ), TRUE)
})
test_that("Default OSQP settings + forced polishing + invalid option => normal", {
  expect_equal(suppressMessages(
    tsbalancing(window(test_ts, start = start(test_ts), end = start(test_ts)), test_specs,
      quiet = TRUE,
      osqp_settings_df = data.frame(polish = TRUE, blabla = 0)
    )$osqp_settings_df$polish
  ), TRUE)
})
...
test_that("Specified bounds and alter coefs", {
  ...
  expect_equal(suppressMessages(
    tsbalancing(test_ts, test_specs,
      quiet = TRUE, temporal_grp_periodicity = 4,
      alter_pos = 1.001,
      alter_neg = 1.002,
      alter_mix = 1.003,
      alter_temporal = 1e-3,
      lower_bound = -888888,
      upper_bound = 888888
    )$prob_val_df[c(1, 33), c("lower_bd", "upper_bd", "alter"), drop = FALSE]
  ), tmp)

  tmp <- data.frame(
    lower_bd = c(-10, -888888),
    upper_bd = c(888888, 888888),
    alter = c(1.001, 1.002)
  )
  attr(tmp, "row.names") <- c(5L, 6L)
  expect_equal(suppressMessages(
    tsbalancing(test_ts, test_specs2,
      quiet = TRUE, temporal_grp_periodicity = 4,
      alter_pos = 1.001,
      alter_neg = 1.002,
      alter_mix = 1.003,
      ...
      lower_bound = -888888,
      upper_bound = 888888
    )$prob_val_df[c(5, 6), c("lower_bd", "upper_bd", "alter"), drop = FALSE]
  ), tmp)
  tmp <- data.frame(
    lower_bd = c(-Inf, -10, -Inf, -Inf),
  ...
  expect_equal(suppressMessages(
    tsbalancing(test_ts, test_specs2,
      quiet = TRUE,
      temporal_grp_periodicity = 4
    )$prob_val_df[c(1, 5, 6, 33), c("lower_bd", "upper_bd", "alter"), drop = FALSE]
  ), tmp)
})
...
test_that("`trunc_to_zero_tol`", {
  ...
  expect_equal(suppressMessages(
    tsbalancing(test_ts, test_specs,
      quiet = TRUE,
      temporal_grp_periodicity = 4
    )$prob_val_df$value_out[c(2, 5, 11, 14)] == 0
...
test_that(paste0(test_str, ": binding tempotal totals, `tolV_temporal`"), {
  expect_type(suppressMessages(
    tsbalancing(test_ts, test_specs3,
      quiet = TRUE,
      temporal_grp_periodicity = 4,
      tolV = 1e-9,
      tolV_temporal = 0.01, tolP_temporal = NA
    )$out_ts
  ), "double")
})
test_that(paste0(test_str, ": nonbinding tempotal totals, `tolV_temporal`"), {
  expect_type(suppressMessages(
    tsbalancing(test_ts, test_specs3,
      ...
      alter_temporal = 1,
      tolV = 1e-9,
      tolV_temporal = 0.001, tolP_temporal = NA
    )$out_ts
  ), "double")
})
test_that(paste0(test_str, ": binding tempotal totals, `tolP_temporal`"), {
  expect_type(suppressMessages(
    tsbalancing(test_ts, test_specs3,
      ...
      temporal_grp_periodicity = 4,
      tolV = 1e-9,
      tolV_temporal = NA, tolP_temporal = 0.001
    )$out_ts
  ), "double")
})
...
test_that("Missing data for a relevant time series => warning", {
  ...
  test_ts2[11, "y"] <- NA
  expect_warning(suppressMessages(
    tsbalancing(test_ts2, test_specs,
      quiet = TRUE,
      temporal_grp_periodicity = 4
    )
  ))
...
test_that("Missing series (specified in the problem specs) => error", {
  expect_error(suppressMessages(
    tsbalancing(test_ts[, c("a", "b")], test_specs,
      quiet = TRUE,
      temporal_grp_periodicity = 4
    )
  ))
...
test_that("Empty problem specs => error", {
  expect_error(suppressMessages(
    tsbalancing(test_ts, test_specs[NULL, , drop = FALSE],
      quiet = TRUE,
      temporal_grp_periodicity = 4
    )
  ))
...
test_that("unsolved fixed problem: 3 warnings (unsolvable problem, constraints not met, final warning)", {
  expect_warning(expect_warning(expect_warning(suppressMessages(
    tsbalancing(
      ...
      rbind(
        test_specs[substr(tolower(test_specs$row), 1, 3) == "con", ],
        data.frame(
          TYPE = c("alter", NA, NA, NA),
          Col = c(NA, "x", "y", "z"),
...
test_that("problem_specs: lowerBd > upperBd", {
  expect_error(
    tsbalancing(
      ts(data.frame(a = 1, b = 1, c = 1), start = 2021, frequency = 4),
      data.frame(
        type = c("eq", NA, NA, NA, "lowerBd", NA, "upperBd", NA),
