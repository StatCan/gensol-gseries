...
When installing **from GitHub**, specify arguments `build_vignettes = TRUE` and `dependencies = TRUE` to also install the _suggested_ G-Series dependencies (packages necessary to build the vignettes that are not installed by default by `remotes::install_github()`):
```{r, eval = FALSE}
install.packages("remotes")
